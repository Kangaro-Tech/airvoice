    ...
    async (req: FastifyRequest<{ Body: DonationBody }>, reply) => {
      ...
      if (body.chq_amount && body.chq_amount > 0) {
        try {
          ...
          if (!catId) { 
            const n = await supabase.from('expense_categories')
              .insert({ name:'Donations', type:'expense' }).select('id').single(); 
            catId = n.data?.id; 
          }
          if (catId) await supabase.from('expenses').insert({ 
            category_id: catId, amount: body.chq_amount,
